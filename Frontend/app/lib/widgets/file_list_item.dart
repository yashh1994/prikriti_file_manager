...
class _FileListItemState extends State<FileListItem> {
  ...
  void initState() {
    super.initState();
    // Initialize with only the filename without extension
    final nameWithoutExtension = _getFileNameWithoutExtension(
  ...
  String _getFileNameWithoutExtension(String fullName) {
    final lastDotIndex = fullName.lastIndexOf('.');
    if (lastDotIndex == -1) {
      return fullName; // No extension found
    }
  ...
  String _combineNameWithExtension(String newName) {
    final originalExtension = widget.file.extension;
                        ...
                        ? Column(
                            ...
                            children: [
                              TextField(
                                ...
                                onChanged: (value) {
                                  setState(() {
                                    _hasValidName = _validateFileName(value);
                                  });
                                },
                                decoration: InputDecoration(
                                  ...
                                  hintText:
                                      'Enter filename (extension preserved)',
                                ...
                                onSubmitted: (value) {
                                  if (value.trim().isNotEmpty &&
                                      _validateFileName(value.trim())) {
                                    // Combine the new name with the original extension
                                    final fullNameWithExtension =
                                        _combineNameWithExtension(value.trim());
                                    widget.onRename(fullNameWithExtension);
                                  }
                              ...
                              Text(
                                'Press Enter to save, Esc to cancel • Extension .${widget.file.extension} will be preserved',
                                style: const TextStyle(
                                  fontSize: 11,
                                  color: Color(0xFF9AA0A6),
