...
export class FileMetadataService {
   ...
   */
  async deleteFileByName(filename: string): Promise<{ success: boolean; message: string }> {
    try {
      const fileInfo = await this.findFileByName(this.uploadsPath, filename);
      ...
      await fs.promises.unlink(fileInfo.fullPath);
  ...
  async getUploadsStats(): Promise<{
    ...
    try {
      ...
      await this.calculateStats(this.uploadsPath, { totalFiles: 0, totalSize: 0, totalDirectories: 0, lastModified: null });

      return {
        success: true,
        message: 'Upload statistics retrieved successfully',
        stats: { totalFiles, totalSize, totalDirectories, lastModified },
      };
    } catch (error) {
      throw new Error(`Failed to get uploads stats: ${error}`);
    }
  }
  ...
  private async calculateStats(
    dirPath: string,
    stats: { totalFiles: number; totalSize: number; totalDirectories: number; lastModified: Date | null }
  ): Promise<void> {
    const items = await fs.promises.readdir(dirPath);
    ...
    for (const item of items) {
      ...
      if (itemStats.isDirectory()) {
        ...
        await this.calculateStats(itemPath, stats);
      } else if (itemStats.isFile()) {
        stats.totalFiles++;
        stats.totalSize += itemStats.size;
        
        if (!stats.lastModified || itemStats.mtime > stats.lastModified) {
          stats.lastModified = itemStats.mtime;
        }
      }
    }
  }
